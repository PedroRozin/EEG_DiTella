...
def main():
    ...
    def show_trial(stim_labels):
        ...
        for label in stim_labels:
            ...
            if label == 11: #este es el target
                while clock.getTime() < stim_dur:
                    if event.waitKeys(maxWait=.002, keyList=['right'], clearEvents=False): #flechita derecha para target
                        explore.set_marker(21)
                        event.clearEvents()
            if label ==10:

                while clock.getTime() < stim_dur:
                    if event.waitKeys(maxWait=.002, keyList=['lctrl'], clearEvents=False): #flechita izquierda para no target
